...
## Now
- [ ] Run `supabase/migrations/0002_events.sql` on `multigames-db` (Supabase dashboard → SQL Editor → paste → Run; idempotent). Blocking: `/api/track` returns 500 on every hub tile click until the table exists, and an authenticated admin still gets 500 from `/api/admin/stats`. Both confirmed live. Verification: `POST /api/track` should answer 204.
