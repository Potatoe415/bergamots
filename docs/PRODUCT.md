...
- Google login status on the hub (`auth.js`): a top-right icon lets a visitor sign in with a Google/Gmail account or sign out. Purely a client-side status (no server session, no stored personal data — see `docs/TECH.md`); it does not currently change the behavior of any game.

Out_Of_Scope:
- Centralized user accounts / persisted identity for players. Players stay anonymous by default. The one optional exception is the hub's Google login status (see Core_Features): it stores no personal data, has no server session, and does not identify a player to any game or backend — it is a UI status only, not an account. The admin stats page remains separately gated by a single shared password, with no account, session, or user record.
