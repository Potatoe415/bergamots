...
- Profile page (`/profile`, `public/profile/`): currently just an editable "Nom" text field, saved client-side. Intended to later pre-fill the name field in games; that wiring does not exist yet — saving the name has no effect on any game so far.
