...
- Google login status on the hub (`auth.js`): a top-right icon lets a visitor sign in with a Google/Gmail account or sign out. When signed in, the popover shows the signed-in email and a "Profil" link to a placeholder profile page (`/profile`, currently empty — no information shown there yet). Purely client-side (no server session — see `docs/TECH.md`); it does not currently change the behavior of any game.
