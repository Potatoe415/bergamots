...
if ($Force) {
    ...
    Add-SyncHistoryEntry -Action "PUSH [FORCE]" -Details "All local files sent (GitHub overwritten)"
    git add -A
...
# Realign, log, and push
git reset origin/main
$pushedCount = (git diff --name-only origin/main..HEAD | Where-Object { $_ -ne "" -and $_ -ne "SYNC-HISTORY.md" }).Count
Add-SyncHistoryEntry -Action "PUSH" -Details "$pushedCount sent, $divergentUpdates received (date resolved)"
git add -A
