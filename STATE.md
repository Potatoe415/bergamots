...
Last_Action: Added a "Président" hub tile (`public/hub-config.json`, new `thumbnail-president.jpg` derived from coinchapp's splash art, pinned in `hub.js`'s `PINNED_GAME_IDS` right after coinche/bouilla) and fixed a regression: coinchapp restructured its own home (`/`) into a 3-game picker, so the hub's `coinche` entry (`launch: ".../"`) was landing on that picker instead of Coinche's own splash — changed to `.../coinche` to match the `.../bouilla`/`.../president` pattern. Verified live: all three `coinchapp.vercel.app/{coinche,bouilla,president}` routes return 200, and the local hub's resolved tile `href`s go straight to each game (no intermediate picker). Not yet committed/pushed — awaiting instruction.
Next_Actions:
