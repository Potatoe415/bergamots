...
Last_Action: Added a "Mode defaite" settings checkbox (checked by default) so players can disable the triple-click defeat-mode feature. New `state.setup.defeatModeEnabled` field, persisted via `yatzy-defeat-mode-enabled` in `localStorage` (`readPersistedDefeatModeEnabled()` defaults to `true` when nothing is stored yet, unlike the other toggles). Wired into `render.js` (`renderSettingsRows`), `app.js` (`handleSettingsInputChange`, `canActivateDefeatMode`, `isDefeatModeTurn`, `resetGame` carryover — unchecking mid-game also force-clears any already-active defeat-mode flag), `session.js` (`handleMatchStarted` carryover), and `i18n.js` (FR/EN/ES). Bumped `yatzy-offline-v31` → `v32` in `sw.js` since `app.js`/`render.js`/`session.js`/`i18n.js` changed. Verified live in browser: checkbox defaults ON, unchecking persists to `localStorage` and flips the pill to OFF, clearing the key restores the ON default. `node --check` passed on all touched files. Committed and pushed to `origin/main`.
Next_Actions:
- Add visual tags (player count, duration, type) to Hub game tiles.
- Ask the tester to hard-refresh/reopen Yatzy once `yatzy-offline-v32` is live and re-test the triple-click (with the new checkbox left at its default ON).
