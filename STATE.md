...
Last_Action: Added the "La Bataille Corse" tile (4th coinchapp game) to the Hub: new entry in `public/hub-config.json` (external, `cartesdes` category, launch `https://coinchapp.vercel.app/bataillecorse`, `sameTab: true`) placed right after "Président", plus `hub.js`'s `PINNED_GAME_IDS` extended to `[coinche, bouilla, president, bataillecorse]` so it sorts next to Président. Generated `public/games/coinchapp/assets/thumbnail-bataillecorse.jpg` (520x932, matching the Président thumbnail's dimensions) via `sharp`, cropped from coinchapp's own `public/bataillecorse-full.jpg`. No `hub.js` launch-URL logic changes needed — `determineTargetUrl`/`appendLaunchParams` already generically append `?name=`/`?avatar=`/`?lang=` to any external launch URL. Not yet committed/pushed — awaiting instruction.
Next_Actions:
