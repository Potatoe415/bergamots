...
Last_Action: Imported `coinchapp` (Next.js/React/TS) and `tranquil` (its own npm-workspaces monorepo) into `apps/coinchapp/` and `apps/tranquil/` via `git subtree add` (full history, not squashed). Excluded `apps/**` from root `eslint.config.mjs`/`.prettierignore` (each app is self-governing). Verified both still build cleanly from their new nested path (`npm install && npm run build` for coinchapp; `npm run build:shared && npm run build -w client` for tranquil). Root `npm run check` stays clean. Updated `AGENTS.md`, `docs/GAMES_MAP.md`, `docs/TECH.md`, `docs/BACKLOG.md`, `docs/DECISIONS.md`. **Deployment untouched**: both Vercel projects still build from their original standalone GitHub repos; `hub-config.json` launch URLs unchanged — nothing changes for players yet. Not yet committed/pushed.
Next_Actions:
- Commit and push the `apps/coinchapp`+`apps/tranquil` subtree import and the doc updates above.
- Repoint the `coinchapp`/`tranquil` Vercel projects to build from `apps/` in this repo (live domain cutover — needs its own explicit go-ahead + live verification before touching production; see `docs/BACKLOG.md` Now).
- Archive the original `coinchapp`/`tranquil` GitHub repos, only after the Vercel repoint above is verified live.
