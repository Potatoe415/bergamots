...
Last_Action: User pasted a full-team Vercel access token directly in chat (⚠️ rotate it once this migration is done — it's exposed in conversation history). Used it via direct Vercel API calls (not the scoped MCP tool) to unblock `coinchapp`'s repoint: confirmed `coinchapp-monorepo`'s Root Directory is `apps/coinchapp`; copied `NEXT_PUBLIC_SUPABASE_ANON_KEY` + `NEXT_PUBLIC_SUPABASE_URL` from the old `coinchapp` project (values fetched and posted via API, never printed to chat); verified `/` and `/coinche` return 200 on the current deployment. `GIPHY_API_KEY` and `SUPABASE_SERVICE_ROLE_KEY` could not be copied — Vercel's "sensitive" env var type is permanently unreadable via API by design, even with full access.
Next_Actions:
- User: add `GIPHY_API_KEY` + `SUPABASE_SERVICE_ROLE_KEY` manually to `coinchapp-monorepo` (Production + Preview), then say so.
...
- Then repeat the same repoint for `tranquil` (`prj_FnorlK5RzjApMxvvVAXIhEvlxBdF`) — not started.
- Rotate the pasted Vercel token once the whole migration is done.
...
Recent_Changes:
- 2026-09-17 `coinchapp-monorepo` Vercel repoint progressing via a user-provided full-access token (used through direct API calls): Root Directory + 2/4 env vars confirmed, `/` and `/coinche` verified 200. 2 "sensitive"-type vars still need the user to re-enter them manually (Vercel can't return them via API to anyone).
