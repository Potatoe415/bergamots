...
Status: Active project on Vercel + Supabase. GameBoy Web hub tile launches the user's Vercel app. Tile art is the original Game Boy photo. Yatzy reactions now include Giphy GIFs next to emojis. `GIPHY_API_KEY` is set on Bergamots Production + Preview. Hub has a Google Sign-In icon (rightmost, real `GOOGLE_CLIENT_ID` from Google Cloud project "muchogames") showing the signed-in email + a "Profil" link. Login confirmed working on `bergamots.vercel.app`, still failing on the custom domain `muchogames.win`. `/profile` has an editable "Nom" field (`bergamots-player-name` in `localStorage`), not yet read by any game.
Current_Goal: Yatzy splash uses the Yam illustration from `tmp-resources`, with Coinchapp-style overlay buttons.
Last_Action: Swapped splash wallpaper to the Yam image (960x1720 WebP, 210 KB). Removed the cream card. Back/settings are glass circles; local/online/join actions are colored pills over the art. SW cache `yatzy-offline-v26`.
Next_Actions:
...
Recent_Changes:
- 2026-09-06 Yatzy splash: Yam wallpaper from `tmp-resources` (210 KB WebP) + Coinchapp-style glass nav and colored overlay buttons; cache `yatzy-offline-v26`.
