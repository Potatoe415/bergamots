...
Last_Action: Found and fixed the actual root cause of the persistent Yatzy overlap report (commit `492ba0e`, pushed, verified live via CDP): `public/games/yatsy/sw.js` caches assets cache-first under a fixed `CACHE_NAME`, so the prior fluid-font-size fix (`a55cb11`) never reached any browser that had already loaded Yatzy once — confirmed via `navigator.serviceWorker.controller` + `document.styleSheets` showing the pre-fix CSS still being served. Bumped `CACHE_NAME` v35→v36. Also fixed a second, independent overlap: `.emoji-trigger-button` floats over `.roll-button`'s corner with no reserved space, so chip "3" collided with it on very narrow viewports (reproduced down to 246px via a maximally-shrunk desktop Chrome window, matching the user's own repro) — added a `padding-right` reservation on `.roll-button` sized to the emoji button's footprint, at both the base and `<=430px` breakpoint sizes. Added a small fixed "v0.1" build tag at the bottom of the Yatzy screen (`data-id="yatsy-build-tag"`, marked `TEMP`) so a stale SW cache is visible at a glance going forward. Verified in the dev browser at 246px/390px: no overlap, `npm run check` passes with zero new warnings.
Next_Actions:
- Ask the user to hard-refresh/reopen Yatzy (or fully close and reopen the tab) so `yatzy-offline-v36` actually takes over, then confirm the overlap is gone by shrinking Chrome again.
- Remember: any future edit to a file in `sw.js`'s `APP_ASSETS` list needs a `CACHE_NAME` bump in the same change, or it will silently never reach already-visited browsers — bump the `v0.1` build tag alongside it as a visible confirmation signal.
